...
export const AuthShell: React.FC = () => {
    ...
    return (
        ...
        >
            {step === 0 && (
                <div className="flex flex-col items-center justify-center w-full h-[100dvh] animate-fade-in relative overflow-hidden px-6 gap-12">

                    {/* Absolutely Positioned Content Group: Pinned exclusively to the top edge to guarantee vertical distances */}
                    <div className="absolute top-[8vh] left-0 right-0 w-full flex flex-col items-center justify-start z-20 px-6 pb-12">
                        {/* Circular Blue Eye Logo */}
                        <div className="relative flex justify-center items-center">
                            {/* Deeper blue outer glow effect */}
                            <div className="absolute inset-0 bg-[#3B82F6] rounded-full blur-[24px] opacity-40 scale-[1.2] w-[72px] h-[72px] mx-auto"></div>
                            <div className="bg-[#007BFF] rounded-full w-[72px] h-[72px] flex items-center justify-center relative z-10 shadow-[0_8px_16px_rgba(31,111,255,0.4)] border border-white/10">
                                <Eye className="w-8 h-8 text-white stroke-[2.5]" />
                            </div>
                        </div>

                        {/* Typography Group (Exactly 1cm / 40px below the logo via explicit margin) */}
                        <div className="text-center w-full" style={{ marginTop: '40px' }}>
                            {/* Header: 'Select Your Language' in large Pure White Serif (Playfair Display) - TWO LINES */}
                            <h1 className="text-[34px] font-heading text-[#FFFFFF] font-bold leading-[1.1]" style={{ textShadow: '0 2px 10px rgba(255,255,255,0.2)' }}>
                                Select Your<br />Language
                            </h1>
                            {/* Sub-text: short instruction in small San-Serif (Inter) centered below, with EXACTLY 1cm (40px) top margin */}
                            <p className="text-[#FFFFFF] opacity-80 text-[14px] font-clinical max-w-[280px] mx-auto leading-snug" style={{ marginTop: '40px' }}>
                                Choose your preferred language to begin your<br />iridology analysis.
                            </p>
                        </div>

                        {/* Language Buttons (Explicitly forced exactly 3cm / 113px below the paragraph) */}
                        <div className="w-full flex flex-col justify-center max-w-[340px] z-10 gap-[20px]" style={{ marginTop: '113px' }}>
                            {/* English: Globe Icon Left, Text Right, centered exactly 1cm (40px) apart */}
                            <button onClick={() => handleLanguageSelect('en')} className="w-full h-[88px] btn-premium-blue flex items-center justify-center gap-[40px] px-6 text-[24px] rounded-2xl">
                                <Globe className="w-7 h-7 opacity-90" />
                                <span className="font-bold tracking-wider pt-1 text-center">English</span>
                            </button>

                            {/* French: Languages (A) Icon Left, Text Right, centered exactly 1cm (40px) apart */}
                            <button onClick={() => handleLanguageSelect('fr')} className="w-full h-[88px] btn-premium-blue flex items-center justify-center gap-[40px] px-6 text-[24px] rounded-2xl">
                                <Languages className="w-7 h-7 opacity-90" />
                                <span className="font-bold tracking-wider pt-1 text-center">Français</span>
                            </button>

                            {/* Arabic: Text Left, Globe Icon Right, centered exactly 1cm (40px) apart (RTL mode) */}
                            <button onClick={() => handleLanguageSelect('ar')} className="w-full h-[88px] btn-premium-blue flex items-center justify-center gap-[40px] px-6 text-[24px] rounded-2xl" dir="ltr">
                                <span className="font-bold tracking-wider pt-2 text-center">العربية</span>
                                <Globe className="w-7 h-7 opacity-90" />
                            </button>

                            {/* Thin Blue Line (Placed exactly 3cm / 113px below the last button) */}
                            <div className="mx-auto w-[113px] h-[2px] bg-[#007BFF] rounded-full mt-[113px]"></div>
                        </div>
                    </div>
                </div>
            )}
